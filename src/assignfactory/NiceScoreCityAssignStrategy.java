...
public class NiceScoreCityAssignStrategy implements AssignStrategy {
    ...
    public void order(Santa santa) {
        ...
        children = children.stream().sorted(Comparator.comparingInt(Child::getId)).collect(Collectors.toList());

        for (Child child : children) {
            map.put(child.getCity(), (double) map.getOrDefault(child.getCity(), 0.0) + child.getNiceScore());
            System.out.println(child.getId() + "  " + child.getNiceScore());
        ...
        for (Cities city : map.keySet()) {
            ...
            System.out.println(city.toString()+ ": " + map.get(city));
        }
